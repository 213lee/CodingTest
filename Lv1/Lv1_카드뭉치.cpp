...
std::string solution(std::vector<std::string> cards1, std::vector<std::string> cards2, std::vector<std::string> goal) {
    ...
    std::vector<std::string> answerGoal;
    ...
    {   
        ...
        else if (!cards2.empty() && cards2.front() == goal[i])
        {   
            cards2.erase(cards2.begin());
        }
        else break;

        answerGoal.push_back(goal[i]);
    }

    return answerGoal == goal ? "Yes" : "No";
}

int main()
{
    std::cout << solution({ "i", "water", "drink" }, { "want", "to" }, {"i", "want", "to", "drink", "water"}) << std::endl;
}