...
예를 들어, 롤케이크에 4가지 종류의 토핑이 올려져 있다고 합시다. 토핑들을 1, 2, 3, 4와 같이 번호로 표시했을 때, 케이크 위에 토핑들이
[1, 2, 1, 3, 1, 4, 1, 2] 순서로 올려져 있습니다. 
...
int solution(std::vector<int> topping) 
{
    ...
    std::unordered_map<int, int> left, right;   //topping의 종류를 key, topping이 나온 횟수 value topping의 길이 최댓값이 크고 데이터의 검색이 자주 일어나지 않기 때문에 unordered_map 사용

    //left map에 모든 topping을 추가
    for (int t : topping) left[t]++;

    //left에서 하나씩 빼서 right에 하나씩 추가하는 방식으로 map size(토핑의 종류)가 같을 때 answer ++
    for (int i = 0; i < topping.size(); i++)
    {
        left[topping[i]]--;
        right[topping[i]]++;

        if (left[topping[i]] == 0) left.erase(topping[i]);

        int lCnt = left.size();
        int rCnt = right.size();

        if (lCnt == rCnt) answer++;
        else if (rCnt > lCnt) break;
    }
...
int main()
{
    ...
    */
    std::vector<int> topping = { 1, 2, 1, 3, 1, 4, 1, 2 };
    std::cout << solution(topping) << std::endl;    
