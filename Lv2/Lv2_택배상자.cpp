...
영재는 택배상자를 트럭에 싣는 일을 합니다. 영재가 실어야 하는 택배상자는 크기가 모두 같으며 1번 상자부터 n번 상자까지 
...
택배상자가 실려 있는 순서가 맞지 않아 배달에 차질이 생깁니다. 따라서 택배 기사님이 미리 알려준 순서에 맞게 영재가 택배상자를 실어야 합니다.
...
(즉, 가장 마지막에 보조 컨테이너 벨트에 보관한 상자부터 꺼내게 됩니다). 보조 컨테이너 벨트를 이용해도 
기사님이 원하는 순서대로 상자를 싣지 못 한다면, 더 이상 상자를 싣지 않습니다.
...
int solution(std::vector<int> order) 
{
    ...
    for (int i = 1; i <= order.size(); i++)
    {
        ...
        else if (!subContainer.empty() && subContainer.top() == i)
        {
            subContainer.pop();
            answer++;
            i--;
    ...
    while (!subContainer.empty() && subContainer.top() == order[answer])
    {
        answer++;
        subContainer.pop();
    }
