...
std::vector<int> solution(std::string msg)
{
    ...
    std::vector<std::string> dictionary = MakeDictionary();
    ...
    while (!msg.empty())
    {
        ...
        {
            std::string sub = msg.substr(0, i);
            auto iter = std::find(dictionary.begin(), dictionary.end(), sub);
...
int main()
{
    ...
    auto iter = std::find(myVector.begin(), myVector.end(), valueToFind);


    std::vector<int> sol = solution("TOBEORNOTTOBEORTOBEORNOT");
    ...
    int p = 1;
    std::string str = "KAKAO";
    std::cout << str.substr(p, str.size() - p) << std::endl;
    str.erase(0, 1);
